...
void BTreeMovieSearch::load(const std::string& filename)
{
    ...
    while (std::getline(file, line)) {
        ...
        if (okYear && okRuntime) {
            btreeMovies.emplace(std::pair<std::string, int>(title.toStdString(), year), Movie(title.toStdString(), year, runtime, genre.toStdString()));
        } else {
            qDebug() << "Skipping invalid year or runtime:" << qLine;
        }
    }
...
void HashMapMovieSearch::load(const std::string& filename)
{
    ...
    while (std::getline(file, line)) {
        ...
        if (okYear && okRuntime) {
            hashmapMovies.emplace(title.toStdString(), Movie(title.toStdString(), year, runtime, genre.toStdString()));
        } else {
            qDebug() << "Skipping invalid year or runtime:" << qLine;
        }
    }
